...
const BIG_HULL_STAGES = [
  { id: 86, name: 'Big Hull: Cuboctahedron', build: (scale) => buildBigHullStage(scale, CUBOCTAHEDRON_CELLS, 3, 3) },
  { id: 88, name: 'Big Hull: Tetrahedral Stack', build: (scale) => buildBigHullStage(scale, TETRAHEDRAL_STACK_CELLS, 4, 4) },
];
...
const BURR_PUZZLE_STAGES = [
  { id: 89, name: 'Burr Puzzle: Key Piece', build: (scale) => buildBigHullStage(scale, CUBOCTAHEDRON_CELLS, 3, 2, [0]) },
  { id: 90, name: 'Burr Puzzle: Key Piece (Tetrahedral)', build: (scale) => buildBigHullStage(scale, TETRAHEDRAL_STACK_CELLS, 4, 3, [0]) },
  { id: 91, name: 'Burr Puzzle: Two Keys', build: (scale) => buildBigHullStage(scale, TETRAHEDRAL_STACK_CELLS, 4, 3, [0, 1]) },
];
...
const BURR_MOLECULE_SPLIT_STAGES = BURR_MOLECULE_SPLIT_STAGE_DEFS.map(({ lobeA, lobeB }, i) => {
  ...
  return {
    ...
    derivedFrom: [{ id: 71, tier: 'Molecule Split' }, { id: 89, tier: 'Burr Puzzle' }],
...
const BURR_MIRRORED_MOLECULE_STAGES = BURR_MIRRORED_MOLECULE_INDEXES.map((shapeIndex, i) => {
  ...
  return {
    ...
    derivedFrom: [{ id: 63, tier: 'Mirrored Molecule' }, { id: 89, tier: 'Burr Puzzle' }],
    build: (scale) => buildBigHullStage(scale, cells, 3, 3, [0]),
  };
});
...
const BURR_BRANCHING_MOLECULE_STAGES = BURR_BRANCHING_MOLECULE_INDEXES.map((defIndex, i) => {
  ...
  return {
    ...
    derivedFrom: [{ id: 79, tier: 'Branching Molecule' }, { id: 89, tier: 'Burr Puzzle' }],
...
const DISPHENOID_KEY_HULL_STAGES = [
  {
    ...
    derivedFrom: [{ id: 86, tier: 'Big Hull' }, { id: 85, tier: 'Disphenoid RD' }, { id: 89, tier: 'Burr Puzzle' }],
    build: (scale) => buildDisphenoidKeyHullStage(scale, CUBOCTAHEDRON_CELLS, 3, 3, 1),
  },
  {
    id: 115,
    name: 'Big Hull: Tetrahedral Stack (Disphenoid)',
    derivedFrom: [{ id: 88, tier: 'Big Hull' }, { id: 85, tier: 'Disphenoid RD' }, { id: 89, tier: 'Burr Puzzle' }],
...
const CRYSTAL_STAGES = [{
  ...
  derivedFrom: [{ id: 86, tier: 'Big Hull' }],
...
export const STAGES = [
  ...
  { id: 85, name: 'Rhombic Dodecahedron (Disphenoids)', build: buildDisphenoidRDStage },
  BIG_HULL_STAGES[0],
  ...
  ...DILUTE_ALLOY_STAGES,
  CARBON_STEEL_STAGE,
  // Multi-Cell relocated here 2026-09-05 (direct instruction: "stage 22
  // is more complex than several of the puzzles following it by amount
  // of pieces to place"): 26 real pieces to place -- the single highest
  // piece count in the entire game, exceeding even the largest Alloy
  // stage (17) -- had been sitting right after the game's own intro
  // tier, ahead of dozens of genuinely simpler stages. No real
  // mechanism (no key/decoy, every piece freely interchangeable), so it
  // sits here by pure piece-count/time-to-complete rather than among
  // the deeper-reasoning crossover tiers right after it -- same
  // "piece count is real difficulty too, not just which mechanism a
  // stage uses" principle already applied to Salt/Calcite's own
  // repositioning above.
  { id: 104, name: 'Multi-Cell', build: buildStage6 },

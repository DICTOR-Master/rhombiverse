...
- **Kite's own second Arrangement (deltoidal trihexagonal tiling)** -- parked 2026-09-23, not implemented. `src/geometry-extensions/lattice-2d.js`'s Kite primitive (hexagon fanned into kites from its own center) reaches all 4 named angles as of this session, and Rhombille already establishes the pattern of a contextual, primitive-specific "Arrangement" toggle for a second real tiling using the same face shape. The deltoidal trihexagonal tiling was floated as Kite's own analogous second arrangement, but scoping it (2026-09-23) found it's real new construction, not a quick extension: it's the dual of the **rhombitrihexagonal tiling** (3.4.6.4 -- triangle, square, hexagon, square around each vertex), which doesn't exist anywhere in this file (only the plain triangular/hexagonal lattice does) -- so it needs a new base tiling built first, then its dual's kite derived from that, not a reuse of anything already shipped. Checked the sibling Polyhedraverse project for a shortcut (`app/lib/polyhedra/miscellaneous/quad-prisms/`, `app/lib/rvcmg/adapters/kiteToUHex.ts`) -- real, rigorously-measured kite geometry exists there, but it's the **DI**/**DH** Catalan-solid kite faces (3D, different vertex configuration), not this 2D Laves tiling's own kite; no direct reuse. If revisited, budget it like building a new primitive from scratch (comparable to Kagome's own first pass), not like extending an existing one.
